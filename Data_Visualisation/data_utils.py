...
# Master function for visualisation:
def print_labels_on_image(image_path, label_path, format = 'YOLO', task = 'instance_segmentation', jupyter_notebook = False):
    '''
    Choose formats as YOLO or COCO
    '''

    if format == 'YOLO' and task == 'instance_segmentation':

        if jupyter_notebook:
            print_yolo_labels_on_image_instance_segmentation_jupyter_notebook(image_path, label_path)
        else:
            print_yolo_labels_on_image_instance_segmentation(image_path, label_path)

    elif format == 'COCO':
        pass

# Visualising an image and labels
def print_yolo_labels_on_image_instance_segmentation(image_path, yolo_label_format):
    '''
    image_path: path to image
    (If image_path is a numpy array, we take first three bands to display the image)
    yolo_label_format: path to label file in YOLO instance segmentation format
    '''
    # Check if the image_path leads to a numpy array
...
# Visualising an image and labels
def print_yolo_labels_on_image_instance_segmentation_jupyter_notebook(image_path, yolo_label_format):
    '''
    function to display an image using matplotlib in jupyter notebook
    image_path: path to image
    yolo_label_format: path to label file in YOLO instance segmentation format
...
def combine_bands_to_single_tif(bands_folder, band_files, band_numbers, output_file, save = True):
    '''
    bands_folder: folder containing all the bands
    band_files: list of band files
    band_numbers: list of band numbers or False to include all bands (like 1, 2, 3, ...)
    ...
    with rasterio.open(f"{bands_folder}/{band_files[0]}") as src:
        ...
        # If save = False:
        # Read each layer and append it to stack but don't write it. return the stack as a numpy array
        if not save:
            stack = np.zeros((len(band_files), src.height, src.width), dtype=src.meta['dtype'])
            for id, layer in enumerate(band_files, start=1):
                try:
                    with rasterio.open(f"{bands_folder}/{layer}") as src1:
                        stack[id-1] = src1.read(1)
                except RasterioIOError:
                    continue
